...
class Datatables_ssp
{
    ...
    static function db_in($conn)
    {
        return self::sql_connect_in($conn);
    }
    ...
    static function simple($request, $table, $primaryKey, $columns, $conn = null)
    {
        ...
        $resFilterLength = self::sql_exec($db, $bindings,"SELECT FOUND_ROWS()"
        );
        ...
        if (!empty($data)) {
            ...
            $output          = self::data_output($columns, $data);
        }
    ...
    static function sql_connect_in($sql_details)
    {
        $_CI = initiateCIs();
        $sql_details = ($sql_details) ? $sql_details : 'default';
        $db = $_CI->load->database($sql_details, TRUE);
     ...
     */
    static function sql_exec($db, $bindings, $sql = null, $xcidb = null)
    {
        if ($xcidb) 
        {
            // Argument shifting
